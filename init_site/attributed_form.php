...
function make_form($Product,$index=0,$quantity=1,$max_quantity=1000000,
  ...
  foreach ($attributes as $i=>$f) {
    $code=to_html($f['code']);
    $code_js=to_js($f['code']);
    $ft=$attr ? $attr[$i] : -1;
    $t=(int)$f['type'];
    $options=$blocks='';
    $options_js=array();
    foreach ($f['options'] as $j=>$o) {
      $code1=to_html($o['code']);
      $code1_js=to_js($o['code']);
      $id="el-$product-$i-$j";
      $pr=$o['price'];
      ...
      $block=(($q=$o['question'])=='') ? '' : 
		"<script>writeBlock('".to_js("$id-block")."','".
			to_js("- ".to_html($q).
				"<br><textarea name='quest[$code][$code1]' cols=20 rows=2 wrap=virtual>".to_html($quest[$i][$j])."</textarea>").
			"',$curr)</script>";
      if ($t<2) {
        $state=$curr ? 'selected' : '';
        $options.="<option ID='$id' value='$code1' $state>$text</option>";
        $blocks.=$block;
        }
      else {
        $state=$curr ? 'checked' : '';
        $options.="<nobr><input ID='$id' type=checkbox name='attr[$code][$code1]' value='1' $state
			onClick='calcOrderForm(this.form)'><label for='$id'>$text</label></nobr><br>$block";
        }
      $options_js[]="'$code1_js',$pr";
      }
    if ($t<2) {
      $state=($ft<0) ? 'selected' : '';
      $options="<select name='attr[$code]' onChange='calcOrderForm(this.form)'>
    ...
    $tmp_js[]="'$code_js',AssocArray('type',$t,'options',AssocArray(".implode(",",$options_js)."))";
    }
